...
const AddForm: React.FC<DataModalProps> = (props) => {
  ...
  const [addFormDateOfAction, setAddFormDateOfAction] = useState(() =>
    nowDate()
  );
  const [addFormDateOfRegister] = useState(() => nowDate());
  const [addFormDateOfReturn, setAddFormDateOfReturn] = useState(() =>
    nowDate()
  );
  ...
  const handleChangeFormCurrency = (value: string) => {
    setAddFormCurrency(value);
  };
      ...
      <Form layout="vertical" id="filter" onSubmit={handleSubmit}>
        <Form.Item label="Název">
          <Input
            ...
            onChange={(e: React.FormEvent<HTMLInputElement>) =>
              setAddFormName(e.currentTarget.value)
            }
            style={{
              width: 150,
            }}
        ...
        {addFormType !== "zapujcka" && (
          <>
            ...
            <Form.Item>
              <Select
                id="addCurrency"
                value={addFormCurrency}
                style={{ width: 150 }}
                onChange={handleChangeFormCurrency}
              >
                <Option value="" disabled>
                  Měna
                </Option>
          ...
          <>
            ...
            <Form.Item>
              <DatePicker
                ...
                value={moment(addFormDateOfReturn)}
              />
            </Form.Item>
          </>
        )}
        ...
        <Form.Item>
          <DatePicker
            ...
            value={moment(addFormDateOfAction)}
          />
        </Form.Item>
